...
class List extends Component {
  ...
  const ventasList = (
    ...
    {this.props.clientes.map(client =>
      <TableRow key={client.id_cliente} selectable={false}>
        <TableRowColumn>{client.id_cliente}</TableRowColumn>
        <TableRowColumn>{client.cedula}</TableRowColumn>
        <TableRowColumn>{client.nombre}</TableRowColumn>
        <TableRowColumn>{client.apellido}</TableRowColumn>
        <TableRowColumn>{client.edad}</TableRowColumn>
        <TableRowColumn>
          <Link to={`/compra/${client.id_cliente}`}><ShoppingCar
            color={cyan500} hoverColor={pinkA200}/>
          </Link>
          <Link to={`/cliente/detail/${client.id_cliente}`}><EyeIcon
            color={blue500} hoverColor={cyan500}/>
          </Link>
          <Link to={`/cliente/update/${client.id_cliente}`}><EditIcon
            color={blue500} hoverColor={cyan500}/>
          </Link>
